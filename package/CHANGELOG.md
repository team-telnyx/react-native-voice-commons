...
- **Suppress the `telnyx.call.incoming` emission for calls already rejected via push action.** When the push payload action is `reject` (Android FCM notification action), `processInvite` now hangs up the incoming call (sending the `telnyx_rtc.bye` to the gateway as before) but no longer emits `telnyx.call.incoming` for that already-ended call. Previously consumers briefly saw a ringing call in `calls$` / their UI for an invite that the SDK was simultaneously hanging up, producing a "ghost ringing" flash. The pending end-action (async) path is suppressed for the same reason.
